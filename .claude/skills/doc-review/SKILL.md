---
name: doc-review
description: "Use instead of writing-plans + executing-plans when implementing a doc-only PRD (no .c/.h files). Runs a 4-step checklist then proceeds directly to editing."
---
...
**Rule:** If ANY file in scope has a `.c` or `.h` extension, STOP immediately and say:
...
Confirm you are inside a git worktree (not the main repo root). If not, enter one now using the `using-git-worktrees` skill or `EnterWorktree` tool before proceeding.
...
4. Smoketest: launch ROM in Emulicious, confirm no pre-existing breakage
